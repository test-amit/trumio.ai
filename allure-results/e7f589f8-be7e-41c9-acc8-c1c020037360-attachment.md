...
- Name: Marketplace Page Tests >> searching for a non-existent team should return no results
- Location: /Users/ajitnayak/Desktop/trumio/tests/specs/marketplace.spec.js:62:3
...
Call log:
  - waiting for locator('input[placeholder="Search team name"]')
    ...
    at /Users/ajitnayak/Desktop/trumio/tests/specs/marketplace.spec.js:69:23
```
...
```ts
  ...
  42 |     await searchInput.waitFor({ timeout: 5000 });
  ...
  49 |     const teamsCard = await page.locator("//div[contains(@class, 'stat-box') and .//p[text()='Teams']]");
  50 |     await teamsCard.click();
  51 |     
  52 |
  53 |     const searchInput = page.getByPlaceholder('Search team name');
  54 |
  55 |
  56 |
  57 |     await searchInput.fill('AI Geeks');
  58 |     await page.keyboard.press('Enter');
  59 |     await expect(page.locator('text=AI Geeks')).toBeVisible();
  60 |   });
  61 |   
  62 |   test('searching for a non-existent team should return no results', async ({ page }) => {
  63 |     // Wait for and click "Teams"
  64 |     await page.waitForSelector('text=Teams', { state: 'visible' });
  65 |     await page.locator('text=Teams').click(); // Click on the "Teams" button first
  66 |
  67 |     // ✅ Now try finding the search input
  68 |     const searchInput = page.locator('input[placeholder="Search team name"]');
> 69 |     await searchInput.click();
     |                       ^ Error: locator.click: Test timeout of 30000ms exceeded.
  70 |   
  71 |     await searchInput.fill('NonExistentTeam123');
  72 |     await page.keyboard.press('Enter');
  73 |   
  74 |     const noResults = page.locator('text=No data found!');
  75 |     await expect(noResults).toBeVisible();
  76 |   });
  77 |   
  78 |   
  79 |   
